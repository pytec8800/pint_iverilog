...
extern "C" int ivl_design_scope(ivl_design_t des, ivl_scope_f func, void *cd) {
    assert(des);
    ivl_scope_t *scopes;
    unsigned nscopes;
    ivl_design_roots(des, &scopes, &nscopes);

    for (unsigned i = 0; i < nscopes; i++) {
        int rc = (func)(scopes[i], cd);
        if (rc != 0)
            return rc;
        rc = ivl_scope_children(scopes[i], func, cd);
        if (rc != 0)
            return rc;
    }
...
extern "C" ivl_expr_t ivl_new_expr(ivl_expr_type_t type, ivl_variable_type_t value, unsigned width, unsigned issigned, unsigned sized)
{
      ivl_expr_t new_expr =  new ivl_expr_s;
      new_expr->type_ = type;
      new_expr->value_ = value;
      new_expr->net_type = NULL;
      //new_expr->file = "";
      new_expr->lineno = 0;
      new_expr->width_ = width;
      new_expr->signed_ = issigned;
      new_expr->sized_ = sized;
      return new_expr;
}

extern "C" void ivl_set_expr_signal(ivl_expr_t expr, ivl_signal_t sig_ , ivl_expr_t word_)
{
    expr->u_.signal_.sig = sig_;
    expr->u_.signal_.word = word_;
}

extern "C" void ivl_set_expr_binary(ivl_expr_t expr, char op, ivl_expr_t left, ivl_expr_t right)
{
    expr->u_.binary_.op_ = op;
    expr->u_.binary_.lef_ = left;
    expr->u_.binary_.rig_ = right;
}
...
extern "C" const char* ivl_event_name_c(ivl_event_t net)
{
      assert(net);
      ...
      const char*sn = ivl_scope_name_c(scope);

      unsigned need = strlen(sn) + 1 + strlen(net->name) + 4;
      if (need > name_size) {
...
extern "C" ivl_signal_t ivl_expr_signal(ivl_expr_t net)
{
	  ...
	  default:
        // printf("Warning! --ivl_expr_signal, can not get signal from this expression.\n");
	    // assert(0);
	    return 0;
      }
}
...
extern "C" unsigned pint_lpm_size(ivl_lpm_t net)
{
    assert(net);
    switch (net->type) {
    case IVL_LPM_MUX:
    ...
    case IVL_LPM_PART_PV:
    {
        ivl_nexus_t nex = ivl_lpm_data(net, 1);
        if(nex) return 2;
        else    return 1;
    }
    default:
...
extern "C" unsigned ivl_scope_func_para_width(ivl_scope_t net, int para_idx) {
    assert(net);
    assert(net->type_ == IVL_SCT_FUNCTION);
    int input_idx = 0;
...
static unsigned scope_name_len(ivl_scope_t net)
{
      ...
      for (ivl_scope_t cur = net ;  cur ;  cur = cur->parent)
	    len += strlen(cur->name_) + 4;

      return len;
}

static void push_scope_basename_c(ivl_scope_t net, char*buf)
{
      assert(net);
      if (net->parent == 0) {
      ...
      push_scope_basename_c(net->parent, buf);
      strcat(buf, "SCP_");
...
extern "C" const char* ivl_scope_name_c(ivl_scope_t net)
{
      assert(net);
      static char*name_buffer = 0;
      static unsigned name_size = 0;
      ...
      push_scope_basename_c(net, name_buffer);
...
extern "C" int ivl_task_need_pc(ivl_scope_t net)
{
      assert(net);
      return task_pc_autosig_info[net]->need_pc;
}

extern "C" int ivl_task_cur_stack_size(ivl_scope_t net)
{
    assert(net);
    return task_pc_autosig_info[net]->need_pc * (sizeof(int) + task_pc_autosig_info[net]->auto_sig_int_len + task_pc_autosig_info[net]->auto_sig_char_len);
}
extern "C" int ivl_task_max_stack_size(ivl_scope_t net)
{
    assert(net);
    return task_pc_autosig_info[net]->max_stack_depth;
}

extern "C" const void* ivl_task_wait_events(ivl_scope_t net)
{
    assert(net);
    return &task_pc_autosig_info[net]->wait_events;
}
...
extern "C" const char* ivl_signal_name_c(ivl_signal_t net)
{
      assert(net);
      static char*name_buffer = 0;
      static unsigned name_size = 0;
      ...
      push_scope_basename_c(net->scope_, name_buffer);
      strcat(name_buffer, "SCP_");
      strcat(name_buffer, net->name_);
