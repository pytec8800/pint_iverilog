...
struct dll_target  : public target_t, public expr_scan_t {
	...
	/* These methods and members are used for forming the
      ...
      int utask_stmt_num;
      int stask_stmt_num;
      vector<ivl_scope_t> tasks_inside_task;
      set<ivl_event_t> events_inside_task;
      ivl_event_t newest_wait_event;
      int nbassign_stmt_num;
      int any_num;
      int pos_num;
      int neg_num;
...
struct ivl_process_s {
      ...
      //mark that whether this loop process is controlled by only one wait event and no other junp structure 
      unsigned only_one_wait_event;
      bool to_lpm_flag;
      bool always_run;
};
...
struct ivl_task_stack_info_s{
    ivl_scope_t scop;
    bool need_pc;
    int auto_sig_int_len;
    int auto_sig_char_len;
    int max_stack_depth;
    vector<ivl_scope_t> inside_tasks;
    set<ivl_event_t> wait_events;
};
/*
