...
vector<ivl_statement_t> dump_stamts;
vector<ivl_statement_t> monitor_stamts;
map<ivl_event_t,int> single_depended_events;
map<ivl_scope_t, ivl_task_stack_info_t> task_pc_autosig_info;

bool dll_target::process(const NetProcTop*net)
{
      bool rc_flag = true;

      ivl_process_t obj = (struct ivl_process_s *)calloc(1, sizeof(struct ivl_process_s));
      ...
      if ((net->type() != IVL_PR_INITIAL) && (delay_stmt_num == 0) && (nbassign_stmt_num == 0) &&
          (wait_stmt_num == 1) && (any_num > 0 && pos_num == any_num && neg_num == any_num))
          obj->to_lpm_flag = true;
      else
          obj->to_lpm_flag = false;

      obj->always_run = 0;
      if ((net->type() != IVL_PR_INITIAL) && (delay_stmt_num == 0) && (utask_stmt_num == 0) &&
          (wait_stmt_num == 1) && (obj->to_lpm_flag == false)) {
          obj->only_one_wait_event = 1;
          if (single_depended_events.count(newest_wait_event) == 0) {
              single_depended_events[newest_wait_event] = 1;
          } else {
              single_depended_events[newest_wait_event]++;
          }
          if (stask_stmt_num || (nbassign_stmt_num > 0 &&
                                 (any_num > 0 && pos_num == any_num && neg_num == any_num))) {
              obj->always_run = 1;
          }
      }
      else{
        obj->only_one_wait_event = 0;
      }
      des_.threads_ = obj;
...
void dll_target::task_def(const NetScope*net)
{
      ...
      utask_stmt_num = 0;
      stask_stmt_num = 0;
      tasks_inside_task.clear();
      events_inside_task.clear();
      ...
      ivl_task_stack_info_t task_stack_info = new ivl_task_stack_info_s;
      task_stack_info->scop = scop;
      task_stack_info->need_pc = (delay_stmt_num || wait_stmt_num);   //this task need pc
      task_stack_info->inside_tasks = tasks_inside_task;
      task_stack_info->wait_events = events_inside_task;
      task_stack_info->max_stack_depth = -1;
      task_stack_info->auto_sig_char_len = 0;
      task_stack_info->auto_sig_int_len = 0;
        
      if (scop->is_auto){
        unsigned auto_sig_len_int = 0;
        unsigned auto_sig_len_char = 0;
        ivl_signal_t augo_sig;
        for (unsigned idx = 0; idx < ivl_scope_sigs(scop); idx += 1){
		  augo_sig = ivl_scope_sig(scop, idx);
          ivl_signal_port_t port_type = ivl_signal_port(augo_sig);
          if ((port_type != IVL_SIP_INPUT)&&(port_type != IVL_SIP_OUTPUT)&&(port_type != IVL_SIP_INOUT)){
            int sig_wid = ivl_signal_width(augo_sig);
            int base_byte_num = 1;
            if (sig_wid <= 4){
                auto_sig_len_char += base_byte_num * ivl_signal_array_count(augo_sig);
            }
            else{
                base_byte_num = (sig_wid + 31) / 32 * 2 * 4;
                auto_sig_len_int += base_byte_num * ivl_signal_array_count(augo_sig);
            }
          }
        }
        task_stack_info->auto_sig_char_len = (auto_sig_len_char + 3) & 0xfffffffc;
        task_stack_info->auto_sig_int_len = auto_sig_len_int;
      }
      
      task_pc_autosig_info[scop] = task_stack_info;
}
...
void dll_target::proc_stask(const NetSTask*net)
{
	...
	/* System task names are lex_strings strings. */
      ...
      stmt_cur_->u_.stask_.scope = lookup_scope_(net->scope());
      ...
      if (!strcmp(net->name(), "$dumpvars"))
          dump_stamts.push_back(stmt_cur_);
      if (!strcmp(net->name(), "$monitor"))
          monitor_stamts.push_back(stmt_cur_);
...
void dll_target::proc_utask(const NetUTask*net)
{
      ...
      tasks_inside_task.push_back(stmt_cur_->u_.utask_.def);
}
	...
	// This event processing code is also in the NB assign above.
      ...
      for (unsigned edx = 0 ;  edx < net->nevents() ;  edx += 1) {
	    ...
	    if (net->nevents() == 1){
          ...
          newest_wait_event = ev_tmp;
          events_inside_task.insert(ev_tmp);
        }
	    else{
